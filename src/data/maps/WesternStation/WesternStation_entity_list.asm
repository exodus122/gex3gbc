    db   ENTITY_TV_BUTTON
    dw   $04b8, $0090
    dw   $04c8, $04a8, $00a0, $0080
    dw   $0001
    db   $24

    db   ENTITY_TV_REMOTE
    dw   $04b8, $0070
    dw   $04c8, $04a8, $0080, $0060
    dw   $0001
    db   $24

    db   ENTITY_TV_BUTTON
    dw   $0048, $02d0
    dw   $0058, $0038, $02e0, $02c0
    dw   $0002
    db   $25

    db   ENTITY_TV_REMOTE
    dw   $0048, $02b0
    dw   $0058, $0038, $02c0, $02a0
    dw   $0002
    db   $25

    db   ENTITY_TV_BUTTON
    dw   $07a0, $0260
    dw   $07b0, $0790, $0270, $0250
    dw   $0003
    db   $25

    db   ENTITY_TV_REMOTE
    dw   $07a0, $0240
    dw   $07b0, $0790, $0250, $0230
    dw   $0003
    db   $25

    db   ENTITY_PAW_COIN
    dw   $0030, $0020
    dw   $0040, $0020, $0030, $0010
    dw   $0001
    db   $04

    db   ENTITY_PAW_COIN
    dw   $01e0, $00c0
    dw   $01f0, $01d0, $00d0, $00b0
    dw   $0002
    db   $24

    db   ENTITY_PAW_COIN
    dw   $0790, $00d0
    dw   $07a0, $0780, $00e0, $00c0
    dw   $0003
    db   $25

    db   ENTITY_BONUS_COIN
    dw   $00e0, $0030
    dw   $00f0, $00d0, $0040, $0020
    dw   $0000
    db   $21

    db   ENTITY_WESTERN_STATION_ENEMY_CACTUS
    dw   $00f0, $0148
    dw   $0240, $0030, $0158, $0138
    dw   $0000
    db   $04

    db   ENTITY_BLUE_FLY_TV
    dw   $0150, $0078
    dw   $0160, $0140, $0088, $0068
    dw   $0014
    db   $04

    db   ENTITY_GREEN_FLY_TV
    dw   $01e0, $00a0
    dw   $01f0, $01d0, $00b0, $0090
    dw   $0000
    db   $04

    db   ENTITY_PURPLE_FLY_TV
    dw   $0248, $0038
    dw   $0258, $0238, $0048, $0028
    dw   $0000
    db   $04

    db   ENTITY_WESTERN_STATION_PLAYING_CARD
    dw   $0260, $0020
    dw   $0270, $0250, $0030, $0010
    dw   $0000
    db   $04

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0028, $0108
    dw   $0030, $0010, $0118, $00f8
    dw   $003c
    db   $21

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0058, $00e0
    dw   $0060, $0030, $00f0, $00d0
    dw   $004b
    db   $21

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0098, $00b8
    dw   $00a0, $0070, $00c8, $00a8
    dw   $005a
    db   $21

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $00c8, $0098
    dw   $00d0, $00a0, $00a8, $0088
    dw   $0069
    db   $21

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $005e, $0150
    dw   $0100, $0050, $0160, $0140
    dw   $0078
    db   $21

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $00a3, $0150
    dw   $0100, $0050, $0160, $0140
    dw   $0078
    db   $21

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $00e8, $0150
    dw   $0100, $0050, $0160, $0140
    dw   $0078
    db   $21

    db   ENTITY_WESTERN_STATION_PLAYING_CARD
    dw   $0130, $0140
    dw   $0140, $0120, $0150, $0130
    dw   $0000
    db   $21

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0230, $00a8
    dw   $0240, $0220, $00b8, $0098
    dw   $0078
    db   $21

    db   ENTITY_GREEN_FLY_TV
    dw   $0170, $0098
    dw   $0180, $0160, $00a8, $0088
    dw   $0000
    db   $21

    db   ENTITY_TUT_TV_SNAKE_FACING_RIGHT
    dw   $0014, $0065
    dw   $0024, $0004, $0075, $0055
    dw   $0000
    db   $22

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0060, $0098
    dw   $0090, $0050, $00a8, $0088
    dw   $003c
    db   $22

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $00d0, $0098
    dw   $0100, $00c0, $00a8, $0088
    dw   $003c
    db   $22

    db   ENTITY_WESTERN_STATION_PLAYING_CARD
    dw   $0020, $0030
    dw   $0030, $0010, $0040, $0020
    dw   $0000
    db   $22

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $0075, $0110
    dw   $0150, $0070, $0120, $0100
    dw   $003c
    db   $24

    db   ENTITY_WESTERN_STATION_BAT
    dw   $00aa, $00a8
    dw   $0120, $00a0, $00b8, $0098
    dw   $0000
    db   $24

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $00e0, $0110
    dw   $0150, $0070, $0120, $0100
    dw   $003c
    db   $24

    db   ENTITY_WESTERN_STATION_BAT
    dw   $0115, $00a8
    dw   $0120, $00a0, $00b8, $0098
    dw   $0000
    db   $24

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $0149, $0110
    dw   $0150, $0070, $0120, $0100
    dw   $003c
    db   $24

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $0278, $0110
    dw   $0360, $0270, $0120, $0100
    dw   $001e
    db   $24

    db   ENTITY_WESTERN_STATION_BAT
    dw   $02a8, $00a8
    dw   $0330, $02a0, $00b8, $0098
    dw   $0000
    db   $24

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $02e8, $0110
    dw   $0360, $0270, $0120, $0100
    dw   $001e
    db   $24

    db   ENTITY_WESTERN_STATION_BAT
    dw   $0320, $00a8
    dw   $0330, $02a0, $00b8, $0098
    dw   $0000
    db   $24

    db   ENTITY_WESTERN_STATION_ROCK_PLATFORM
    dw   $0358, $0110
    dw   $0360, $0270, $0120, $0100
    dw   $001e
    db   $24

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $03c0, $0108
    dw   $03e0, $03c0, $0118, $00f8
    dw   $005a
    db   $24

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0400, $0108
    dw   $0420, $0400, $0118, $00f8
    dw   $005a
    db   $24

    db   ENTITY_WESTERN_STATION_PLAYING_CARD
    dw   $0440, $00a0
    dw   $0450, $0430, $00b0, $0090
    dw   $0000
    db   $24

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0058, $0090
    dw   $0060, $0040, $00a0, $0080
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $00c8, $0090
    dw   $00d0, $00b0, $00a0, $0080
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0128, $0300
    dw   $0130, $0110, $0310, $02f0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0188, $0300
    dw   $0190, $0170, $0310, $02f0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0348, $0200
    dw   $0350, $0330, $0210, $01f0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0688, $0050
    dw   $0690, $0670, $0060, $0040
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $06e8, $0190
    dw   $06f0, $06d0, $01a0, $0180
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0698, $0190
    dw   $06a0, $0680, $01a0, $0180
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $04e8, $0240
    dw   $04f0, $04d0, $0250, $0230
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0388, $02e0
    dw   $0390, $0370, $02f0, $02d0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $03f8, $02e0
    dw   $0400, $03e0, $02f0, $02d0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0488, $02e0
    dw   $0490, $0470, $02f0, $02d0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0508, $02e0
    dw   $0510, $04f0, $02f0, $02d0
    dw   $003c
    db   $25

    db   ENTITY_WESTERN_STATION_HARD_HAT
    dw   $0558, $02e0
    dw   $0560, $0540, $02f0, $02d0
    dw   $003c
    db   $25

    db   ENTITY_GREEN_FLY_TV
    dw   $06f8, $0248
    dw   $0708, $06e8, $0258, $0238
    dw   $0000
    db   $25

    db   ENTITY_WESTERN_STATION_PLAYING_CARD
    dw   $0410, $0290
    dw   $0420, $0400, $02a0, $0280
    dw   $0000
    db   $25

    db   ENTITY_WESTERN_STATION_RISING_PLATFORM
    dw   $0768, $00b8
    dw   $0778, $0758, $0130, $00b0
    dw   $0080
    db   $25

    db   ENTITY_LIST_TERMINATOR
